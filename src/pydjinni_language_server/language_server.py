...
def init_language_server(config: Path, generate_on_save: bool, generate_base_path: Path, log: Path):
    ...
    def validate(ls, uri):
        ...
        try:
            ...
            error_items = [
                to_diagnostic(error, DiagnosticSeverity.Error, f"{error.__doc__}: {error.description}")
                for error in e.items
                if isinstance(error.position.file, TextDocumentPath) and error.position.file.document.uri == uri
            ]
            refs = e.type_refs
        ...
        ast_cache[uri] = [type_def for type_def in ast if type_def.position and type_def.position.file.as_uri() == uri]
        type_def_cache[uri] = [
            type_def for type_def in defs if type_def.position and type_def.position.file.as_uri() == uri
        ]
        for ref in refs:
            if (
                isinstance(ref.position.file, TextDocumentPath)
                and ref.position.file.document.uri == uri
                and ref.type_def
                and ref.type_def.deprecated
            ):
        ...
        hover_cache[uri] = to_hover_cache(
            [
                ref
                for ref in refs
                if isinstance(ref.position.file, TextDocumentPath) and ref.position.file.document.uri == uri
            ],
            [
                file_ref
                for file_ref in file_imports
                if isinstance(file_ref.position.file, TextDocumentPath) and file_ref.position.file.document.uri == uri
            ],
            [
                field
                for field in fields
                if isinstance(field.position.file, TextDocumentPath) and field.position.file.document.uri == uri
            ],
            ast,
        )

        dependency_cache[uri] = set(
            [
                ref.type_def.position.file.as_uri()
                for ref in refs
                if ref.type_def
                and isinstance(ref.type_def, BaseType)
                and ref.type_def.position
                and ref.type_def.position.file
                and ref.type_def.position.file.as_uri() != uri
            ]
    ...
    def hover(ls: LanguageServer, params: HoverParams):
        ...
        if (
            ...
            and cache_entry.type_def.comment
        ):
            return Hover(
            ...
            cache_entry
            and (
                isinstance(cache_entry, BaseField)
                or isinstance(cache_entry, BaseType)
                or isinstance(cache_entry, Namespace)
            )
            and cache_entry.comment
        ):
            return Hover(
                contents=MarkupContent(
                    kind=MarkupKind.Markdown,
            ...
            return Hover(
                contents=MarkupContent(kind=MarkupKind.Markdown, value=f"```txt\n{cache_entry.path}\n```"),
                range=identifier_range(cache_entry),
            )
    ...
    def definition(ls: LanguageServer, params: DefinitionParams):
        ...
        if (
            ...
            return Location(uri=cache_entry.type_def.position.file.as_uri(), range=type_range(cache_entry.type_def))
        elif cache_entry and isinstance(cache_entry, FileReference):
            return Location(uri=cache_entry.path.as_uri(), range=Range(Position(0, 0), Position(0, 0)))
                    ...
                    if hasattr(type_def, target.key) and type_def.position:
                        ...
                        if hasattr(target_type, "header"):
                            file_output_path: Path = (
                                generate_base_path / main_generator.header_path / target_type.header
                            )
                        else:
                            file_output_path: Path = (
    ...
    def execute_command(ls: LanguageServer, arguments):
        ...
        ls.show_document_async(ShowDocumentParams(uri=file_path))
