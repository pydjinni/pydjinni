...
class TestRecord {
    @Test
    void testGetRecord() {
        var foo = new Foo(true, (byte) 8, (short) 16, 32, 64, 32.32f, 64.64, "test string", new ArrayList<>(List.of(0, 1)), new ArrayList<>(List.of("foo", "bar")));
    ...
    void testToString() {
        var foo = new Foo(true, (byte) 8, (short) 16, 32, 64, 32.32f, 64.64, "test string", new ArrayList<>(List.of(0, 1)), new ArrayList<>(List.of("foo", "bar")));
        assertEquals("test.record.Foo{booleanT=true,byteT=8,shortT=16,intT=32,longT=64,floatT=32.32,doubleT=64.64,stringT=test string,intList=[0, 1],stringList=[foo, bar]}", foo.toString());
    }
